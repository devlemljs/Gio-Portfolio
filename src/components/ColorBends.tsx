...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    // Quick fallback/safety for mouseInfluence typo in parameters mapping
    (material.uniforms.uMouseInfluence as { value: number }).value = mouseInfluence;
    ...
    const loop = () => {
      ...
      const r_angle = (deg * Math.PI) / 180;
      const cosVal = Math.cos(r_angle);
      const sinVal = Math.sin(r_angle);
      material.uniforms.uRot.value.set(cosVal, sinVal);
  ...
  return (
    <div 
      ...
      style={{ ...style, minHeight: "100%", minWidth: "100%", display: "block" }} 
    />
  );
}
